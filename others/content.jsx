...
chrome.storage.sync.get(['selectedRole', 'comments'], (result) => {
  ...
  if (validPage) {
    injectReactApp(role, comments);
  }
});

function injectReactApp(role, comments) {
  if (document.getElementById('content-react-root')) {
    return; // Prevent multiple injections
  }

  const container = document.createElement('div');
  container.id = 'content-react-root';
  ...
  ReactDOM.render(<ContentApp role={role} initialComments={comments} />, container);
}
